...
// node_modules/@midnight-ntwrk/wallet-sdk-hd/dist/index.js
var Roles = {
  NightExternal: 0,
  NightInternal: 1,
  Dust: 2,
...
var HDWallet = class _HDWallet {
  ...
  static fromSeed(seed) {
    try {
      ...
      return { type: "seedOk", hdWallet: new _HDWallet(rootKey) };
    } catch (e) {
...
var generateMnemonicWords = (strength = 256) => mnemonicToWords(generateMnemonic(wordlist, strength));
var joinMnemonicWords = (mnenomic) => mnenomic.join(" ");
var generateRandomSeed = (strength = 256) => {
  return crypto.getRandomValues(new Uint8Array(Math.ceil(strength / 8)));
};
var validateMnemonic2 = (mnemonic) => validateMnemonic(mnemonic, wordlist);
...
function testMnemonicGeneration() {
  ...
  try {
    ...
    const isValid = validateMnemonic2(joinMnemonicWords(words));
    return {
...
function testMnemonicValidation() {
  ...
  try {
    ...
    const isValid = validateMnemonic2(mnemonic);
    const invalidMnemonic = "invalid words that are not a real mnemonic phrase";
    const isInvalid = !validateMnemonic2(invalidMnemonic);
    return {
